...
// ================================================
// Basic struct definitions from poke.h assumed:
...
void evolvePokemon(OwnerNode *owner)
{
	...
	if(searchPokemonBFS(owner->pokedexRoot,ID) != NULL)
	{
		...
		if(searchPokemonBFS(owner->pokedexRoot, newID) == NULL)
		{
		...
		{
			printf("Evolution ID %d (%s) already in the Pokedex.\nReleasing %s (ID %d)",
				   newID, pokedex[newID - 1].name,
				   pokedex[ID-1].name, ID);
		}
	}
	else
